...
def prompt(which):
    if which == 1:
        entry = input("Enter your starting point 0-5: ")
        if entry.isnumeric():   #for out purposes, and non number is q
            return int(entry) % 6
        else:
            return 'Q'
    else:   #and other which is assumed to start $ prompt for now
...
def path_for_goal(pred, start, goal):
    if start == goal:
        print("Trade your", goal)
    else:
        path_for_goal(pred, start, pred[goal])
        print("then get", goal)
    pass
...
def main():
    ...
    graph = {'Book':{'Record':5, 'Poster':0},
             'Record':{'Bass Guitar':15, 'Drums':20},
    ...
    for i in range(len(graph.keys())):
        print(i, ":", list(graph.keys())[i])

    entry = prompt(1)
    start = list(graph.keys())[entry]
    pred, table = Dijkstra(graph, start, 'Piano')
    print("table: ", table)
    ...
    while entry != 'Q':
        ...
        print("you should buy {0} for ${1}".format(table[subi][0],
                                                   table[subi][1]))
        print("You need to take the following steps:")
        path_for_goal(pred, start, table[subi][0])
