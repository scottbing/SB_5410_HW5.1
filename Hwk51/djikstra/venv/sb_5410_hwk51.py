...
def main():
    ...
    print("graph: ", graph)     # print graph for verification

    # Calling the function with source an target
    Dijkstra(graph, 'Gilroy', 'Lubbock')
    graph = construct_graph()
    Dijkstra(graph, 'Gilroy', 'Zanesville')
    graph = construct_graph()
    Dijkstra(graph, 'Tupelo', 'Fargo')
    graph = construct_graph()
    Dijkstra(graph, 'Worcester', 'Gilroy')
